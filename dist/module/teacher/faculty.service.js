...
const getFacultyById = (_id) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield faculty_model_1.default.findOne({ _id }).populate("userId");
    return result;
});
const updateFaculty = (_id, updateData) => __awaiter(void 0, void 0, void 0, function* () {
    const session = yield mongoose_1.default.startSession();
    session.startTransaction();
    try {
        const teacher = yield faculty_model_1.default.findOne({ _id }).session(session);
        if (!teacher) {
            throw new AppError_1.default(http_status_codes_1.StatusCodes.FORBIDDEN, "Faculty not found");
        }
        // Other updates for faculty details...
        const updatedStudent = yield faculty_model_1.default.findOneAndUpdate({ _id }, updateData, { new: true, runValidators: true, session });
        if (!updatedStudent) {
            throw new Error("Failed to update faculty");
        }
        const userUpdateData = {};
        const updateStudentData = yield faculty_model_1.default.findById(_id).session(session);
