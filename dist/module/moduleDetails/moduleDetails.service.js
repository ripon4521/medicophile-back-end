...
const createModuleDetails = (payload) => __awaiter(void 0, void 0, void 0, function* () {
    const module = yield modules_model_1.default.findOne({ _id: payload.moduleId });
    const course = yield course_model_1.default.findOne({ _id: payload.courseId });
    if (payload.content_type === "Exam" ||
        payload.content_type === "Lecture" ||
        payload.content_type === "Notes") {
        const exam = yield exam_model_1.default.findOne({ _id: payload.contentId });
        const lecture = yield lecture_model_1.default.findOne({ _id: payload.contentId });
        const notes = yield notes_model_1.default.findOne({ _id: payload.contentId });
        if (payload.content_type === "Exam" && !exam) {
            throw new AppError_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "Invalid content id or exam id");
        }
        else if (payload.content_type === "Lecture" && !lecture) {
            throw new AppError_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "Invalid conetnt or lecture id");
        }
        else if (payload.content_type === "Notes" && !notes) {
            throw new AppError_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "Invalid conetnt or notes id");
        }
    }
    if (!module) {
        throw new AppError_1.default(http_status_codes_1.StatusCodes.BAD_REQUEST, "Invalid module id");
    }
    if (!course) {
