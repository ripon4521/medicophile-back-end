...
const getAllAdmin = (query) => __awaiter(void 0, void 0, void 0, function* () {
    const courseQuery = new querybuilder_1.default(admin_model_1.default, query)
        .search(["name", "role"])
        .filter()
        .sort()
        .paginate()
...
const updateAdmin = (_id, updateData) => __awaiter(void 0, void 0, void 0, function* () {
    const session = yield mongoose_1.default.startSession();
    session.startTransaction();
    try {
        const teacher = yield admin_model_1.default.findOne({ _id }).session(session);
        if (!teacher) {
            throw new AppError_1.default(http_status_codes_1.StatusCodes.FORBIDDEN, "admin not found");
        }
        // Other updates for faculty details...
        const updatedStudent = yield admin_model_1.default.findOneAndUpdate({ _id }, updateData, { new: true, runValidators: true, session });
        if (!updatedStudent) {
            throw new Error("Failed to update admin");
        }
        const userUpdateData = {};
        const updateStudentData = yield admin_model_1.default.findById(_id).session(session);
        ...
        session.endSession();
        throw new AppError_1.default(http_status_codes_1.StatusCodes.FORBIDDEN, "Error updating admin and user");
    }
});
const deleteAdmin = (_id) => __awaiter(void 0, void 0, void 0, function* () {
    const session = yield mongoose_1.default.startSession();
    session.startTransaction();
    try {
        // Faculty ke find kore check kora
        const faculty = yield admin_model_1.default.findOne({ _id }).session(session);
        if (!faculty) {
        ...
        yield admin_model_1.default.findOneAndUpdate({ _id }, {
            isDeleted: true,
...
const getSingleAdmin = (_id) => __awaiter(void 0, void 0, void 0, function* () {
    const result = yield admin_model_1.default.findOne({ _id }).populate("userId");
    return result;
});
