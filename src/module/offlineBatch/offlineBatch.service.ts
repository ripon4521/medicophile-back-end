...
const getAllOfflineBatch = async (query: Record<string, unknown>) => {
  ...
  if (!result) {
    throw new AppError(StatusCodes.BAD_REQUEST, "Failed  to get Offline Batch ");
  }
  return result;
};
...
const updateOfflineBatch = async (slug: string, payload: Partial<IOfflineBatch>) => {
  ...
  const deleteOffLineBatch = async (slug: string) => {
    ...
    if (!result) {
      throw new AppError(StatusCodes.BAD_REQUEST, "Faile to delete PLease Try Again ");
    }
    return result;
  };
