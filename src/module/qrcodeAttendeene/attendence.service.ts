...
console.log(student)
  ...
  if (existingAttendance) {
    throw new AppError(StatusCodes.CONFLICT, "Attendance for this student in the same batch already exists for today.");
  }
...
const singleAttendance = async(_id:string) => {
    const result = await Attendence.findOne({_id}) .populate([
        {
          path: "batchStudent",
          populate: [
            {
