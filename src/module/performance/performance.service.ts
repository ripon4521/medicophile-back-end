...
import CqMarkingModel from '../cqMarking/cqMarking.model';
import GapAttempModel from '../gapsAttemp/gapAttemp.model';
import Attendence from '../qrcodeAttendeene/attendence.model';
import { IMcqAttemp } from '../mcqAttemp/mcqAttemp.interface';
...
type StudentData = {
  mcqs: { accuracy: number };
  cqs: { averageScore: number };
  gaps: { percentage: number };
  attendance: { attendancePercentage: number };
};

// Helper functions to calculate performance metrics
const calculateAccuracy = (mcqs: MCQ[]): number => {
  const totalCorrect = mcqs.reduce((sum, item) => sum + (item.correctCount ?? 0), 0); // Default to 0 if undefined
  const totalWrong = mcqs.reduce((sum, item) => sum + (item.wrongCount ?? 0), 0); // Default to 0 if undefined
  return (totalCorrect / (totalCorrect + totalWrong)) * 100;
};

const calculateAverageScore = (cqs: CQ[]): number => {
  const totalScore = cqs.reduce((sum, item) => sum + item.score, 0);
  return totalScore / cqs.length;
};

const calculateGapPercentage = (gaps: Gap[]): number => {
...
const calculateAttendancePercentage = (attendance: Attendance[]): number => {
...
const generatePerformanceReport = (studentData: StudentData): string => {
...
export const getStudentPerformance = async (studentId: string): Promise<any> => {
  ...
  const [mcqs, cqs, gaps, attendance] = await Promise.all([
    McqAttemptModel.find({ studentId }),
    CqMarkingModel.find({ studentId }),
    GapAttempModel.find({ studentId }),
    Attendence.find({ studentId, isDeleted: false }), // Ensure `isDeleted` is false
  ]);

  // Helper function to map the raw MCQ data into a proper structure
  const mapToMCQ = (mcqs: IMcqAttemp[]): MCQ[] => {
    return mcqs.map((mcq) => ({
      correctCount: mcq.correctCount ?? 0,  // Default to 0 if undefined
      wrongCount: mcq.wrongCount ?? 0,     // Default to 0 if undefined
    }));
  };
