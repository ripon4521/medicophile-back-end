...
const updatePurchaseToken = async(_id:string, payload:IPurchaseToken) => {
    ...
    if (!result) {
        throw new AppError(StatusCodes.BAD_REQUEST, "Failed to update purchase token")
    }
    return result;
}
...
const deletePurchaseToken = async(_id:string) => {
    ...
    const result = await PurchaseTokenModel.findOneAndUpdate({_id}, {
        isDeleted:true,
    ...
    if (!result) {
        throw new AppError(StatusCodes.BAD_REQUEST, "Failed to delete purchase token")
    }
    return result;
}
