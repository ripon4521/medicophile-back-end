...
whatsAppRouter.post(
  "/send-whatsapp",
  validateRequest(whatsappMessageSchema),
...
whatsAppRouter.post("/webhook", async (req, res) => {
  ...
  const message =
    incomingMessage?.entry[0]?.changes[0]?.value?.messages[0]?.text?.body;
  const sender =
    incomingMessage?.entry[0]?.changes[0]?.value?.messages[0]?.from;
