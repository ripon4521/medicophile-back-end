...
const createCqQuestion = async (payload:ICqQuestion) => {
    ...
    if (!result) {
        throw new AppError(StatusCodes.BAD_REQUEST , "Failed to create CQ Question, Please check and try again")
    }
    return result;
}
...
const getALlCqQuestion = async(query: Record<string, unknown>) => {
  ...
  if (!result) {
    throw new AppError(StatusCodes.BAD_REQUEST , "Failed to get Cq Question data. Please check your qury and try again. Query only question filed")
  }
  return result;
}
...
const updateCqQuestion = async (_id:string, payload:ICqQuestion) => {
    ...
    if (!update) {
     throw new AppError(StatusCodes.BAD_REQUEST, "Failed to  update Cq Question , Please check and try again")  
    }
    return update;
}

const deleteCqQuestion = async( _id:string ) => {
    const result = await CqQuestionModel.findOneAndUpdate({_id}, {
        isDeleted:true,
    ...
    if (!result) {
        throw new AppError(StatusCodes.BAD_REQUEST, "Failed to delete Cq Question. Please check and try again")
    }
    return result;
}
