...
const createCqAttemps = async (payload: ICqAttemps) => {
    ...
    try {
      ...
      if (question.durationDate && createdAttempt.submittedTime > question.durationDate) {
...
const getAllCqAttemps = async() => {
    const result = await CqAttempModel.find({isDeleted:false}).populate('studentId').populate('examId');
    if (!result) {
