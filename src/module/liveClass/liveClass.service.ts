...
const createLiveClass = async (payload: ILiveClass) => {
  ...
  const getAllLiveClass = async () => {
    const get = await LiveClassModel.find({isDeleted:false}).populate('courseId').populate('createdBy');
    if (!get) {
      throw new AppError(
        StatusCodes.BAD_REQUEST,
  ...
  const deleteLiveClass = async (slug:string) => {
    const get = await LiveClassModel.findOneAndUpdate({slug}, {
        isDeleted:true,
  ...
  const singleGetLiveClass = async (slug:string) => {
    const get = await LiveClassModel.findOne({slug}).populate('courseId').populate('createdBy');
    if (!get) {
      throw new AppError(
        StatusCodes.BAD_REQUEST,
