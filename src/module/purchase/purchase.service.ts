...
const createPurchase = async (payload: IPurchase) => {
    ...
    try {
      ...
      const purchaseToken = await PurchaseTokenModel.findOne({ _id: payload.purchaseToken }).session(session);
      const issuedBy = await UserModel.findOne({ _id: payload.issuedBy }).session(session);
...
const deletePurchase = async (_id:string) => {
    const result = await PurchaseModel.findOneAndUpdate({_id}, {
        isDeleted:false,
