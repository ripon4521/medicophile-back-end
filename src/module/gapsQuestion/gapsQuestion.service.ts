...
const cretaeGapsQuestion = async(payload:IGapsQuestion) => {
    ...
    if (!result) {
        throw new AppError(StatusCodes.BAD_REQUEST, "Failed to create Gaps Question. Please check and try again")
    }
    return result;
}
...
const getAllGapsQuestion = async(query: Record<string, unknown>) => {
  ...
  if (!result) {
    throw new AppError(StatusCodes.BAD_REQUEST, "Failed to get Gaps Questions. Please check and try again")
}
  return result;
...
const updateGapsQuestion = async(_id:string,payload:IGapsQuestion) => {
    ...
    if (!result) {
        throw new AppError(StatusCodes.BAD_REQUEST, "Failed to update gaps question. Please check id and try again")
    }
    return result;
}
...
const deleteGapsQuestion = async(_id:string) => {
    const result = await GapsQuestionModel.findOneAndUpdate({_id}, {
        isDeleted:true,
    ...
    if (!result) {
        throw new AppError(StatusCodes.BAD_REQUEST, "Failed to delete gaps question. Please check id and try again")
    }
    return result;
}
