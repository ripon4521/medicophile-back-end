...
const ObjectIdSchema = z.string().refine((val) => Types.ObjectId.isValid(val), {
 ...
 const createGapsQuestionSchema = z.object({
    ...
    answer: z.array(z.string()).nonempty({ message: "At least one answer is required" }),
  });
  ...
  const updateGapsQuestionSchema = z.object({
    ...
    answer: z.array(z.string()).nonempty({ message: "At least one answer is required" }).optional(),
  });
