...
const getAllGapAttemp = catchAsync(async (req, res) => {
  ...
  const getSpecificUserAttemp = catchAsync(async(req, res) => {
    ...
    if (!studentId || !examId) {
        throw new AppError(StatusCodes.BAD_REQUEST, "Please provide studentId and Exam Id")
    }
    const result = await gapAttempService.getSpecificUserGapsAttempMark(studentId,examId);
    sendResponse(res, {
        statusCode: StatusCodes.OK,
        message: "Gap Attemp fatched successfully",
        data: result,
