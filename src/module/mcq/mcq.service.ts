...
const updateMcq = async(_id:string, payload:IMcqQuestion) => {
    ...
    if (!result) {
        throw new AppError(StatusCodes.BAD_REQUEST, "Failed to update mcq  question")
    }
    return result;
...
const deleteMcq = async(_id:string) => {
    ...
    const result = await McqQuestion.findOneAndUpdate({_id}, {
        isDeleted:true,
    ...
    if (!result) {
        throw new AppError(StatusCodes.BAD_REQUEST, "Failed to delete mcq  question")
    }
    return result;
