...
const verifyRefreshToken = async (req:Request, res:Response, next:NextFunction) => {
  ...
  if (!refreshToken) {
    return next(new AppError(StatusCodes.UNAUTHORIZED, "Refresh token missing"));
  }
  ...
  try {
    ...
    if (!user) {
      return next(new AppError(StatusCodes.UNAUTHORIZED, "Invalid refresh token"));
    }
    ...
    return next(new AppError(StatusCodes.UNAUTHORIZED, "Invalid or expired refresh token"));
  }
};
